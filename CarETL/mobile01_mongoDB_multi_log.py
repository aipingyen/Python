...
def getSubContent(url):
    ...
    for page in range(1,pages+1):    #此評論的所有的內頁
        ...
        #                 logger.info("tm",tm)
                    ...
                    logger.error("getSubContent err message : ", e) #
                    reply_no -= 1
                    pass
            # logger.info("real article_no : ",len(replies) + 1)
            # logger.info("repliesOfPage",replies)
        except Exception as e:
            logger.error("getSubContent err message : ",e) #
            pass
...
def update_to_mongodb(collection,url, reply_no):
    ...
    try:
        ...
        logger.error("failed: update_to_mongodb : ", e) #
        # conn.rollback()
