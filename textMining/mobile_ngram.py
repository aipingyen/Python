...
def cutSentence(string, keywords):  ##放入原始文章路徑, 增加斷詞的list
    ...
    for keyword in keywords:  # 清除關鍵字
        line = "".join(string.split(keyword))
    print(type(string))
    for word in string:
        # print(word)
...
def main(month_from, month_end):
    ...
    try:
        ...
        longTermFreq = longTermPriority(total_word, 7, 2)
        # print(longTermFreq)
        with open("C:\\Users\\BIG DATA\\Desktop\\n-gram_vw2014-2.txt", 'w', encoding='utf-8') as fw:
...
if __name__ == '__main__':
    ...
    month_from = datetime(2014, 1, 1, 0, 0)
    month_end = datetime(2014 + 1, 1, 1, 0, 0)
    main(month_from, month_end)
