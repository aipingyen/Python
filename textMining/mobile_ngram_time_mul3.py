...
#     with open("C:\\Users\\BIG DATA\\Desktop\\n-gram2.txt", 'w', encoding='utf-8') as fw:
#         for k, v in longTermsFreq:               # 長詞+次數分配
#             fw.write("{},{}".format(k, v) + '\n')


def multi(text_list, i, minFreq):
    results = []
    freeze_support()
    pool = Pool(8)
    # cpus = cpu_count()
    # for i in range(0, 8):
    result = pool.apply_async(ngram, args=(text_list, i, minFreq )) #, callback=writeInCSV
...
def longTermPriority(string, maxTermLength, minFreq):
    longTermsFreq = []
    text_list = cutSentence(string)  # return 斷句的 list
    print(len(text_list))
    for i in range(maxTermLength, 1, -1):   #7次
        words_freq = multi(text_list, i, minFreq)  #把斷句的list丟到ngram  去切大小算頻率

        for word_freq in words_freq:
            # print("word_freq : ",word_freq)
            longTermsFreq.append(word_freq.get())#.get()
            # print word_freq

    return longTermsFreq

def main(month_from, month_end, year):
    ...
    count = collection.find({"Board":"Volkswagen","tm":{"$gte":month_from,"$lt":month_end}}).count()
    print("筆數 : ", count)
    comment = collection.find({"Board":"Volkswagen","tm":{"$gte":month_from,"$lt":month_end}})
    # print(comment)
    start = time.time()
    total_word = ""
    i = 0
    try:
        for idx in range(count):
            i += 1
            print(comment[idx]['title'])
            content = catch_allContent(comment[idx])  #return allContent list
            data = content[0].replace('\r\n', '').replace('\r', '').replace('\n', '')
            total_word += data
            print("          ",i)
            # sys.stdout.write('\r' +'筆數 :'+str(i))
            printStr = 'now is going on ' + str(idx + 1) + ',which is ' + str(math.floor((idx + 1) / count * 100)) + str('% finished')
            sys.stdout.write('\r' + printStr)
        longTermsFreq =longTermPriority(total_word, 7, 2)   #longTermsFreq =
        print("longTermsFreq : ",longTermsFreq)
        with open("C:\\Users\\BIG DATA\\Desktop\\n-gram_vw{}.txt".format(year), 'w', encoding='utf-8') as fw:
            # for longTermFreq in longTermsFreq.items():
            for k, v in longTermsFreq[0]:
                fw.write("{},{}".format(k, v) + '\n')
...
if __name__ == '__main__':
    # month = datetime.datetime(2000, 1, 1, 0, 0)
    ...
    # main(month)
    # for year in range(2008, 2016, 1):  2008  2009  2010  2011  2012  2013  2014  2015  2016
    year = 2015 #16
    month_from = datetime.datetime(year, 1, 1, 0, 0)
    month_end = datetime.datetime(year+1, 1, 1, 0, 0)
    main(month_from, month_end, year)
